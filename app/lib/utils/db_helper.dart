...
class DBHelper {
  ...
  static Future<void> insertLabel(MyLabel label) async {
    (await _database).insert(
      _labelsTableName,
      label.toMap(),
      conflictAlgorithm: sql.ConflictAlgorithm.replace,
  ...
  static Future<void> updateLabel(MyLabel label) async {
    (await _database).update(
      _labelsTableName,
      label.toMap(),
      where: 'id = ?',
  ...
  static Future<List<MyLabel>> getLabels() async {
    final labelMaps = await (await _database).query(_labelsTableName);

    return List.generate(
      labelMaps.length,
      (index) {
        return MyLabel.fromMap(labelMaps[index]);
      },
    );
  }
